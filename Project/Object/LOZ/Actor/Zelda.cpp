...
Zelda::Zelda()
{
	...
	AddAction();
	//curAction = actions[ANIM_ALREADYDEAD];
...
void Zelda::Update()
{
	...
	if (state == WALK)
	{
		this->translation += moveDirect * speed * Time::Delta();
	}
...
void Zelda::AddAction()
{
	...
	actions.emplace(ANIM_ALREADYDEAD, new Animation(file, 7, 9));
	actions[ANIM_ALREADYDEAD]->SetIndex({ 56 + 4 }, false);

	actions.emplace(ANIM_REVIVE, new Animation(file, 7, 9));
	actions[ANIM_REVIVE]->SetPart(56+4, 56, false);
	actions[ANIM_REVIVE]->SetEndEvent(bind(&Zelda::Walk, this));
